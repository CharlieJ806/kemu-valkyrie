...
### B1 答题限时 60 秒
- 每道题 60 秒倒计时（所有战斗含 Boss），答对后下一题重新计时；倒计时条用现成素材 `fill-timer.webp` / `fill-timer-low.webp`（最后 10 秒变红 + 音效提示）。
- 超时 = 按答错处理：连击清零、反伤、展示正确答案后进入出牌阶段；飘字「⏰ 超时！」。
- 计时器为内存态（不写存档），读档续战时当前题重新计时 60 秒。
- 文件：`lib/store.ts`（`timeoutQuestion`）、`components/screens/BattleScreen.tsx`（倒计时 UI）、`app/cards.css`、`lib/audio.ts`（可选 tick 音效）。
